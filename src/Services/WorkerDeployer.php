...
class WorkerDeployer {
	...
	public function deploy( array $config ): array {
		...
		if ( ! $verify['success'] ) {
			return array(
				'success' => false,
				'message' => __( 'Invalid API token', 'thachpham-offload-cdn-cloudflare-r2' ),
				'steps'   => $steps,
			);
		}
		$steps[] = array(
		...
		if ( ! $upload['success'] ) {
			...
			return array(
				...
				'message' => sprintf(
					...
					__( 'Failed to upload Worker script: %s', 'thachpham-offload-cdn-cloudflare-r2' ),
					$error_msg
				),
				'steps'   => $steps,
			);
	...
	private function configure_route( string $domain ): array {
		...
		if ( ! $zone_id ) {
			return array(
				...
				'message' => sprintf(
					...
					__( 'Zone not found for domain: %s', 'thachpham-offload-cdn-cloudflare-r2' ),
					$base_domain
				),
			);
		}
		...
		return array(
			'success' => false,
			'message' => $result['errors'][0]['message'] ?? __( 'Failed to configure route', 'thachpham-offload-cdn-cloudflare-r2' ),
		);
	}
}

...
class CloudflareAPI {
	...
	public function validate_cdn_dns( string $cdn_url ): array {
		...
		if ( empty( $parsed['host'] ) ) {
			return array(
				'success' => false,
				'message' => __( 'Invalid CDN URL', 'thachpham-offload-cdn-cloudflare-r2' ),
			);
		}
		...
		if ( ! $zone_id ) {
			return array(
				...
				'message' => sprintf(
					...
					__( 'Domain "%s" not found in your Cloudflare account', 'thachpham-offload-cdn-cloudflare-r2' ),
					$base_domain
				),
			);
		}
		...
		if ( $record ) {
			...
			if ( empty( $record['proxied'] ) ) {
				$warnings[] = __( 'DNS record exists but Cloudflare proxy is DISABLED. Worker will NOT work without proxy.', 'thachpham-offload-cdn-cloudflare-r2' );
			}
		...
		if ( $create_result['success'] ) {
			return array(
				...
				'message' => sprintf(
					...
					__( 'DNS record created for %s with Cloudflare proxy enabled.', 'thachpham-offload-cdn-cloudflare-r2' ),
					$cdn_host
				),
				'zone_id' => $zone_id,
			);
		...
		return array(
			'success' => false,
			'message' => $create_result['errors'][0]['message'] ?? __( 'Failed to create DNS record', 'thachpham-offload-cdn-cloudflare-r2' ),
		);
	}
